...
| `marstek_up`                            |                     | 1 if recent parseable MQTT telemetry is healthy, 0 otherwise |
...
| `marstek_scrape_errors_total`           |                     | Total failed polls, including publish failures and cases where no recent parseable telemetry was available by the timeout deadline |
| `marstek_poll_timeouts_total`           |                     | Polls after which no recent parseable telemetry was available by the timeout deadline |
...
`marstek_up` reflects recent parseable MQTT telemetry health, not a strict synchronous `cd=1` request/response handshake. Some batteries keep publishing valid status asynchronously after a poll, so `marstek_up` remains `1` while telemetry is still arriving within the freshness window. `marstek_last_payload_timestamp_seconds` and `marstek_payload_fresh` expose the underlying cache age independently, and cloud reachability is tracked via `marstek_cloud_last_report_timestamp_seconds`.
...
# No recent parseable MQTT telemetry is available:
marstek_up == 0
...
| `MARSTEK_RESPONSE_TIMEOUT`     | `--response-timeout`     | `8s`         | Max wait for an immediate poll response before falling back to recent-telemetry health |
...
Publishing `cd=1` to the command topic encourages the battery to emit a fresh status payload on the device topic, and the exporter does this on every `--poll-interval`. In practice some batteries also continue publishing parseable status asynchronously on their own cadence, so the exporter treats recent background telemetry as healthy even if a given poll does not get an immediate reply inside `--response-timeout`.

The status payload is a flat `key=value,key=value,...` string. The exporter splits it on `,`, maps known keys to Prometheus metrics, and updates the gauges. A parseable payload sets `marstek_up=1`, refreshes `marstek_last_payload_timestamp_seconds`, and keeps `marstek_payload_fresh=1` until `metric_ttl` expires. If a poll does not get an immediate reply within `--response-timeout` but recent parseable telemetry is still available, the exporter keeps the device healthy and suppresses timeout noise. Only when there is neither an in-window response nor sufficiently recent telemetry does it set `marstek_up=0`, increment `marstek_scrape_errors_total`, and track the event in `marstek_poll_timeouts_total`.
